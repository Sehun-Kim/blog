...
#### 위 방식으로 파트너와 나는 10월 말부터 1월초까지 약 2개월에 걸쳐 콘솔 프로젝트를 완성할 수 있었다.
...
확실하게 영역을 분리해 놓으니 테스트 케이스를 만들 때부터 최대한 다른 영역(Model, View)에 의존적이지 않게 설계를 하게 되었다. 이렇게 되니 도메인 로직 자체에 대한 코드를 수정할 필요가 없었고 이미 테스트를 거친 코드를 그대로 사용할 수 있었다.
...
###### 우리는 핵심 도메인이 있는 모델 부분과 입출력과 요청에 따른 분기처리를 하는 컨트롤러 부분이 분리되도록 설계하였다.

일단 클래스의 개수부터가 달랐다. 전에는 딱 4개의 클래스로 모든 기능을 처리했지만, 이번에는 패키지의 개수만 해도 6개였다.
