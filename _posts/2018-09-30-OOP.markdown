...
#### 클래스 vs 객체
- **객체** 란 유일무이하게 `식별`이 가능한 것을 말한다.
- **클래스** 란 객체들의 `분류`이다.

보통 클래스와 객체의 차이를 답하라 하면 많은 사람들이 `붕어빵`과 `붕어빵틀`의 예시를 든다. 일반적으로 `Class`를 생성하여 `Object`를 만들 수 있다 생각하기 때문에 `붕어빵틀`로 `붕어빵`을 만들 수 있다는 것이다.
...
`붕어빵틀`로 `붕어빵`을 만들었는데 타입이 `붕어빵틀`이란 것은 말이 안되는 코드다. `붕어빵틀`은 `붕어빵`에 대한 **클래스**가 아니라 `붕어빵`에 대한 **팩토리클래스**이다. (팩토리 클래스는 다른 객체를 생성하기 위한 클래스다.)
...
### <a id="2-2"></a> 2-2 Java 프로그램의 실행흐름
Link : [Java 프로그램의 실행흐름]({{ site.url }}/JVM)

### <a id="2-3"></a> 2-3 OOP 4대 특성
### <a id="2-4"></a> 2-4 OOP 설계 5원칙
