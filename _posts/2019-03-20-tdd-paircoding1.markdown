...
***페어코딩과 TDD 방식으로 개발했던 프로젝트를 통해 좋은 코드가 무엇인지도 몰랐던 초보 개발자가 무엇을 개선할 수 있었는지 알아보자.***
...
# 초보 개발자 TDD를 만나다.
갈피를 잡지못하고 방황하던 작년 7월, 주니어 개발자들의 아이돌(?) [이동욱님의 글](https://jojoldu.tistory.com/277?category=689637)을 보고 '**코드스쿼드 웹백엔드 코스**'를 알게 되었다. 해당 교육이 내게 좋은 전환점이 될 것 같아, 그해 9월부터 코드스쿼드를 다니기 시작했다.

나는 코드스쿼드를 통해 존경하는 **박재성(a.k.a. javajigi)** 님을 만나 TDD, 객체 지향적 설계, 리팩토링을 통한 클린코드 작성법, 개발자의 학습방법 등을 배울 수 있었다.

짧지만 강렬한 6개월 동안 많은 것을 배웠지만 가장 인상 깊었던 것은 **테스트 주도 개발 방식**을 통해 코드의 설계를 개선한 경험이다.
